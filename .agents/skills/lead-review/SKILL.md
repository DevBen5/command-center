...
description: |
  ...
  le AGENTS.md racine et celui du module touché.
...
Il n'y a pas de base de connaissances séparée sur ce projet : la doc vit dans les `AGENTS.md`.

---

## 3. Charger le contexte du dépôt

Le `AGENTS.md` racine est déjà en contexte. **Lis en plus le `AGENTS.md` du ou des modules touchés**
(`app/modules/<module>/AGENTS.md`, `app/core/<domaine>/AGENTS.md`) : ils portent les invariants
...
⚠️ **La liste fait autorité dans le `AGENTS.md` racine — elle en compte huit, pas trois.** La
...
- [ ] Aucun test ne touche le réseau (hors exceptions déjà documentées dans le `AGENTS.md` du module)
