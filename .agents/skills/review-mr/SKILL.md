...
description: |
  ...
  Enrichit le contexte via la task YouTrack (projet CC) et les `AGENTS.md` des modules touchés.
...
> base **`master`**, doc dans les **`AGENTS.md`**, et **pas de Discord**.
...
l'article racine — la liste grandit, ne pas la figer), qui **synthétisent** les `AGENTS.md`, la
mémoire de travail et le backlog — avec des pointeurs, jamais en copie. Elle peut éclairer le contexte (`search_articles`), mais **les `AGENTS.md`
restent l'autorité** : celui de la racine, et surtout **celui de chaque module touché**
(`app/modules/<module>/AGENTS.md`), qui porte les invariants précis et les pièges. Un verdict
...
`AGENTS.md` racine — elle en compte huit, pas trois** ; ce qui suit n'en garde que les plus
...
- **Couleurs : uniquement les tokens `@theme`**, aucune couleur en dur. ⚠️ **La liste fait
  autorité dans `inertia/css/app.css`, pas dans une doc** — l'énumération de `AGENTS.md` (`bg`,
...
- `AGENTS.md` racine et par module : encouragés, c'est **là** que vit la doc de ce projet
- `README.md` à la racine : autorisé
- Les `TICKET-*.md` existants sont des archives — ne pas en créer de nouveaux
- Un changement non trivial qui laisse le `AGENTS.md` du module muet sur un nouveau piège est un
  point à lever
...
- ⚠️ **La doc peut mentir** — elle décrit parfois un comportement que le code ne fait plus. Tout
  claim tiré d'un `AGENTS.md` sur lequel repose un verdict se revérifie contre le code
