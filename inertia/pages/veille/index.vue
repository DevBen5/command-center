...
function applyFilters(patch: Partial<Filters>): void {
  router.get(
    '/veille',
    {
      ...
      readingQueue: patch.readingQueue !== undefined ? patch.readingQueue : props.filters.readingQueue,
...
<template>
  ...
  <div class="mb-4 flex items-center gap-3">
    ...
    >
    <button type="button" class="rounded-[9px] border border-line-2 bg-panel-2 px-3.5 py-2 text-[12.5px]" @click="submitSearch">
      Rechercher
    </button>
  </div>

  <div class="grid min-h-[560px] grid-cols-[222px_1fr_286px] overflow-hidden rounded-[14px] border border-line bg-panel">
    <!-- Sources / Tags -->
    <div class="border-r border-line bg-bg-2">
      ...
      <div class="flex flex-wrap gap-1.5 p-3">
        <button
          ...
          :class="filters.tag === tag ? 'border-accent bg-accent-soft text-txt' : 'border-line-2 bg-panel-2 text-txt-2'"
    ...
    <div class="min-w-0">
      <div class="flex items-center gap-2 border-b border-line p-4 text-[12px] font-semibold">
        Flux agrégé
        <span class="rounded-full border border-line-2 bg-panel-2 px-2.5 py-0.5 text-[11px] font-normal text-txt-2">
          {{ items.length }} éléments
        </span>
      </div>
      <div v-for="item in items" :key="item.id" class="flex items-center gap-3 border-b border-line px-4 py-3.5">
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11.5px] text-txt-2">
            ...
            <span v-for="tag in item.tags" :key="tag" class="rounded-full border border-line-2 bg-panel-2 px-2 py-0.5 text-[10.5px]">
              #{{ tag }}
            </span>
          </div>
        </div>
      ...
      <div v-if="items.length === 0" class="p-6 text-center text-[13px] text-txt-2">Aucun résultat.</div>
    </div>
    ...
    <div class="border-l border-line bg-bg-2">
      ...
      <div class="flex flex-col gap-2 p-3">
        <div v-for="item in readingQueueItems" :key="item.id" class="rounded-[9px] border border-line bg-panel p-2.5">
      ...
      <form class="flex flex-col gap-2 p-3" @submit.prevent="submitCapture">
        <select v-model="capture.type" class="rounded-md border border-line-2 bg-panel px-2 py-1.5 text-[12px]">
