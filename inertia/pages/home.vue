...
interface Cards {
  services: { up: number; total: number; down: string[]; highRam: { name: string; ram: number | null }[] }
  ...
  <div class="grid grid-cols-2 gap-5">
    ...
    <div class="overflow-hidden rounded-[14px] border border-line bg-panel">
      <div class="flex items-center gap-3 border-b border-line px-[18px] py-4">
        <div class="grid h-[30px] w-[30px] place-items-center rounded-lg border border-line-2 bg-accent-soft text-accent">◧</div>
        <h3 class="text-[15px] font-semibold">Services</h3>
        <span class="ml-auto font-mono text-[11.5px] text-txt-2">
          {{ cards.services.up }} actifs · {{ cards.services.down.length }} arrêté{{ cards.services.down.length > 1 ? 's' : '' }}
        </span>
      </div>
      <div class="px-[18px] py-2">
        ...
        >
          <span class="min-h-[34px] w-[3px] shrink-0 self-stretch rounded-[3px] bg-accent shadow-[0_0_10px_var(--color-accent)]"></span>
          <div class="flex-1">
    ...
    <div class="overflow-hidden rounded-[14px] border border-line bg-panel">
      ...
      <div class="px-[18px] py-2">
        ...
        <div v-if="!cards.agents.failed.length && !cards.agents.running.length" class="py-3.5 text-[12.5px] text-txt-2">
    ...
    <div class="overflow-hidden rounded-[14px] border border-line bg-panel">
      ...
      <div class="px-[18px] py-2">
        <Link href="/veille" class="flex items-center gap-3.5 border-b border-line py-3.5">
          <span class="min-h-[34px] w-[3px] shrink-0 self-stretch rounded-[3px] bg-line-2"></span>
          <div class="flex-1">
            <div class="text-[13.5px] font-semibold">File de lecture — {{ cards.veille.queue }} éléments</div>
        ...
        <Link href="/veille" class="flex items-center gap-3.5 py-3.5">
          <span class="min-h-[34px] w-[3px] shrink-0 self-stretch rounded-[3px] bg-line-2"></span>
          <div class="flex-1">
            <div class="text-[13.5px] font-semibold">{{ cards.veille.untagged }} captures sans tag</div>
      ...
      <div class="px-[18px] py-2">
        <Link href="/revision" class="flex items-center gap-3.5 border-b border-line py-3.5">
          <span
            ...
            :class="cards.leitner.due ? 'bg-accent shadow-[0_0_10px_var(--color-accent)]' : 'bg-line-2'"
          ></span>
          <div class="flex-1">
            <div class="text-[13.5px] font-semibold">{{ cards.leitner.due }} cartes à réviser aujourd'hui</div>
        ...
        <Link href="/revision" class="flex items-center gap-3.5 py-3.5">
          <span class="min-h-[34px] w-[3px] shrink-0 self-stretch rounded-[3px] bg-line-2"></span>
          <div class="flex-1">
            <div class="text-[13.5px] font-semibold">{{ cards.leitner.total }} cartes en mémoire</div>
