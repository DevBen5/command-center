 ...
 * exclusif à `/corpus` : le flux d'ingestion (case « conserver ») n'a aucun
  ...
  <AppModal v-slot="{ titleId }" @close="emit('cancel')">
    <div class="mt-16 w-[420px] rounded-[14px] border border-line bg-panel p-4">
      <div :id="titleId" class="text-[14px] font-semibold">
        {{ t('corpus.cours.conflict.title') }}
      </div>
      <p class="mt-2 text-[12.5px] text-txt-2">
        {{ t('corpus.cours.conflict.body', { title: existingTitle }) }}
      </p>
      <div class="mt-4 flex flex-col gap-2">
        <button
          ...
          @click="emit('replace')"
        >
          {{ t('corpus.cours.conflict.replace') }}
        </button>
        <button
          type="button"
        ...
        >
          {{ t('corpus.cours.conflict.createSecond') }}
        </button>
        <button
          type="button"
          ...
          @click="emit('cancel')"
        >
          {{ t('corpus.cours.conflict.cancel') }}
        </button>
      </div>
    </div>
  </AppModal>
