...
coffre). Voir « Les médias Immich » plus bas et `app/modules/veille/AGENTS.md`.
...
structurelle que `providers/veille_provider.ts` (voir le `AGENTS.md` racine, point 7) — un
...
raison structurelle que `commands/reset_account.ts` (voir le `AGENTS.md` racine, point 7) — le
...
dans le `AGENTS.md` RACINE**, section « Une seule modale dans tout le dépôt » (append du
...
- ⚠️ **Le chassis ne porte AUCUNE structure interne** (pas de bandes en-tête/corps/pied) : c'est
  ...
  sur le corps défilant — voir le `AGENTS.md` du module Leitner pour ce que chaque classe tient).
...
3. **une clé survit à la révocation de la session qui l'a ouverte**, jusqu'à son échéance — mais
   **inatteignable** : le cookie porteur du `keyId` est mort, et `AuthMiddleware` expulse avant
   ...
   dépendre le **noyau** d'un module détachable (point 7 du `AGENTS.md` racine).
...
(`app/modules/veille/AGENTS.md`, « Immich — les vidéos du téléphone »). Une entrée peut référencer
...
⚠️ **Table dédiée, jamais un préfixe de plus sur `dedup_key`.** `app/modules/veille/AGENTS.md`
...
Le `AGENTS.md` racine explique pourquoi le chiffrement du coffre ne dérive **pas** d'`APP_KEY` :
...
Repris tel quel de `BACKUP_MIRROR_DIR`/`AGENTS_CONFIG_PATH` (voir le `AGENTS.md` racine, « Les
...
et `backups/` (voir le `AGENTS.md` racine, « Les données ») ; la seconde paie le CPU d'ImageMagick
...
son AGENTS.md — « le catalogue est en clair »). Deux jeux de données différents.
...
Les cinq points du `AGENTS.md` racine s'appliquent tels quels. Deux propres à ce module :
