...
plus bas et le `AGENTS.md` racine, point 7) : `modules.has('leitner')` avant toute requête, pour
...
autre copie. `./pgdata` survit à un `docker compose down -v` (voir le `AGENTS.md` racine), pas à une
...
documenté dans le `AGENTS.md` RACINE**, section « Une seule modale dans tout le dépôt » — il vit
dans `inertia/components/`, c'est du châssis et non le bien d'un module ; le `AGENTS.md` du coffre
...
générique logé chez un module détachable est exactement la faute que le point 7 du `AGENTS.md`
...
`/corpus/glossaire`. La description actuelle est dans `app/modules/corpus/AGENTS.md`.
...
de code **sans** réintroduire de `v-html`. Voir le `AGENTS.md` racine, section « Le seul `v-html`
...
- ⚠️ **`taxonomy_combobox.spec.ts` ne prouve quelque chose que parce qu'il tape d'abord** : `filtering`
  vaut déjà `false` au montage. C'est le piège de tout test de composant — voir le `AGENTS.md` racine.
