...
import { Box, Search, TriangleAlert } from 'lucide-vue-next'
...
const props = defineProps<{ dockerDisponible: boolean; services: Service[]; stats: Stats }>()
...
<template>
  ...
  <!-- Hors service : ce déploiement n'a pas accès à Docker (config/docker.ts, CC-116).
       Rien d'autre ne se rend — des cartes lues en base seraient des conteneurs imaginaires. -->
  <div
    v-if="!dockerDisponible"
    class="rounded-[13px] border border-warn/40 bg-linear-to-b from-warn/10 to-panel p-5"
  >
    <div class="flex items-center gap-3">
      <TriangleAlert :size="18" :stroke-width="1.5" aria-hidden="true" class="shrink-0 text-warn" />
      <h2 class="text-[15px] font-semibold">{{ t('services.offline.title') }}</h2>
    </div>
    <p class="mt-2 text-[13px] leading-relaxed text-txt-2">{{ t('services.offline.body') }}</p>
  </div>
  ...
  <template v-else>
    ...
    <div class="mb-[18px] flex items-center gap-3">
      ...
      <span
        class="rounded-full border border-line-2 bg-panel-2 px-3 py-1.5 text-[11px] text-txt-2"
        >{{ t('services.toolbar.category') }}</span
      >
      <span
        class="rounded-full border border-line-2 bg-panel-2 px-3 py-1.5 text-[11px] text-txt-2"
        >{{ t('services.toolbar.status') }}</span
      >
