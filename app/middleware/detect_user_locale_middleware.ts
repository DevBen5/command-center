...
export default class DetectUserLocaleMiddleware {
  ...
  protected getRequestLocale(ctx: HttpContext) {
    // 1. Choix explicite de l'utilisateur, mémorisé dans un cookie.
    const cookieLocale = ctx.request.cookie('locale')
    if (cookieLocale && i18nManager.supportedLocales().includes(cookieLocale)) {
      return cookieLocale
    }

    // 2. Sinon, meilleure correspondance avec l'en-tête Accept-Language du navigateur.
