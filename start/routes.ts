...
router
  .group(() => {
    ...
    */
    ...
    |
    */
    // ⚠️ Module désactivable (CC-137) : `MODULES` sans `services`/`agents` fait que ces
    // routes n'existent PAS — 404, pas 403. `modules` est lu une seule fois, au démarrage
    // (préchargement de ce fichier) : voir `config/modules.ts`.
    ...
    // ⚠️ Module désactivable (CC-137) : sans `veille` dans `MODULES`, ce groupe entier
    // n'existe pas — 404 sur toute route `/veille/*`, pas 403.
    ...
    // ⚠️ Module désactivable (CC-137) : sans `leitner` dans `MODULES`, tout `/revision/*`
    // n'existe pas — 404, pas 403.
