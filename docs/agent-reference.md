...
son contenu et n'ajoute que ce qui manque — voir `app/modules/leitner/AGENTS.md`. ⚠️ **Depuis
...
app/modules/  services · agents · veille · leitner · corpus · coffre  → import via #modules/*
  └── controllers/ models/ migrations/ seeders/ services/ validators/ pages/
app/bridges/  intégrations entre modules                    → import via #bridges/*
providers/    app_url · installation · leitner · agents · veille · backup · coffre → import via #providers/*
commands/     reset_account                         → import via #commands/*
```

- **Les alias de `package.json` décrivent exactement l'arborescence réelle** : `#core/*`,
  `#modules/*`, `#bridges/*`, `#providers/*`, `#commands/*`, plus `#tests/*`, `#start/*`, `#config/*`. Les douze alias hérités du
...
- **Une clé écrite dans un template doit exister, et c'est désormais tenu par un test** (CC-113).
  ...
  la vraie fusion — extrait les clés littérales de tous les `.vue` du dépôt, châssis
  ...
  - ⚠️ **Les clés calculées échappent par construction** — ``t(`agents.status.${status}`)``,
    `t(filter.labelKey)`. Aucune extraction statique ne les
...
`inertia/i18n/`. Le documenter dans le `AGENTS.md` d'un module serait le ranger là où personne ne
...
figure dans sa liste de specs transverses. Le `AGENTS.md` de chaque module envoie lire son index
...
**L'apparence se vérifie au navigateur, et nulle
...
- **ImageMagick (vignettes NAS du coffre, CC-228) : `execFile` en tableau, coder TOUJOURS forcé
  ...
  ImageTragick (MVG/MSL déguisés en image) reste ouverte. Voir le `AGENTS.md` du coffre pour la
  mesure complète.
- **Le seul `v-html` du dépôt passe par `renderMarkdown`, et rien d'autre** (CC-133,
  ...
  **Cinq** `v-html` existent — le verso dans `leitner/pages/index.vue`, les deux champs de
  ...
  de la liste blanche de `markdown_renderer.ts`, jamais du texte d'une carte. Voir le `AGENTS.md`
...
- **Le coffre a sa propre porte, et elle ne se recopie pas ailleurs** (CC-178,
  `app/modules/coffre/AGENTS.md`). Deux points de ce fichier y sont **volontairement enfreints**,
...
> relais entre conversations, mémoire et compaction — vivent dans les instructions globales de
> l'outil et valent
> partout. Ici, seulement ce qui est propre à Command Center.

### Les skills du dépôt (`.agents/skills/`)
...
- **`/triage-youtrack`**, **`/create-issue-from-code`**, **`/summarize-sprint`**,
  **`/link-commit-to-issue`** (2026-08-04) — quatre skills légers autour du
  ...
  `In Progress`. Il n'a jamais été invoqué une seule fois (`.agents/youtrack-usage.log` n'existait
  même pas), tout en coûtant sa `description` dans le contexte système de **chaque** session.

- **`/youtrack-stats`** (2026-08-04) — agrège `.agents/youtrack-usage.log` (non versionné), alimenté
...
articles `CC-A-1` (sommaire) à `CC-A-13`, accès par le serveur MCP officiel. Avec les `AGENTS.md` du
